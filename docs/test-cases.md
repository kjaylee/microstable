...
- 입력:
  ...
  - 대상 담보 가격이 10틱 연속
    ...
    조건 충족
- 기대출력:
  - 10틱 연속 조건이 충족되기 전에는 `CB1.active=True` 유지
  - 10틱 연속 충족 시점에 `CB1.active=False`
- 판정기준(PASS/FAIL):
  - PASS: 연속성 조건(10틱) 충족 시에만 복구
  - FAIL: 10틱 이전 조기 복구 또는 10틱 이후 미복구

### TC-CB012
- ID: TC-CB012
- 카테고리: Circuit Breaker
- 설명: 동시 CB-1 + CB-2 발동 시 우선순위(CB-2 우선) 검증
- 입력:
  - 동일 윈도우에서 CB-1 조건(단일 담보 디페그)과 CB-2 조건(다중 담보 스트레스) 동시 충족
- 기대출력:
  - 두 CB 모두 활성화되더라도 정책 적용 우선순위는 `CB-2 > CB-1`
  - 민팅 정책은 CB-2 규칙(민팅 중단)이 최종 적용
  - CB-1 완화 동작은 상위 CB-2 활성 중 보류
- 판정기준(PASS/FAIL):
  - PASS: 최종 동작이 CB-2 우선순위를 따름
  - FAIL: CB-1 완화 동작이 우선 적용되거나 충돌 발생

### TC-CB013
- ID: TC-CB013
- 카테고리: Circuit Breaker
- 설명: CB 쿨다운 기간 내 재발동 방지
- 입력:
  - 임의 CB가 복구되어 `COOLDOWN` 상태 진입
  - 복구 후 5틱 이내 동일 CB 발동 조건을 재주입
- 기대출력:
  - 쿨다운 5틱 동안 동일 CB 재발동 불가
  - 5틱 경과 후 동일 조건 재주입 시 재발동 가능
- 판정기준(PASS/FAIL):
  - PASS: 쿨다운 윈도우 내 재발동 0회
  - FAIL: 5틱 이내 재발동 발생

### TC-CB014
- ID: TC-CB014
- 카테고리: Circuit Breaker
- 설명: 30틱 내 3회 발동 시 장기 안전모드 전환 검증
- 입력:
  - 동일 CB를 30틱 윈도우 내 3회 발동시키는 이벤트 시퀀스
- 기대출력:
  - 상태가 `EXTENDED_ACTIVE`로 전환
  - 해당 CB의 최소 유지시간이 `min_hold × 3`으로 증가
- 판정기준(PASS/FAIL):
  - PASS: 3회/30틱 조건에서 장기 안전모드 정확히 발동
  - FAIL: 전환 누락 또는 유지시간 배수 규칙 미적용

### TC-CB015
- ID: TC-CB015
- 카테고리: Circuit Breaker
- 설명: 복구 역순(CB-4 → CB-3 → CB-2 → CB-1) 강제 검증
- 입력:
  - `CB1, CB2, CB3, CB4`가 모두 활성인 복합 스트레스 상태
  - 각 CB의 복구 조건을 모두 충족시키는 정상화 시퀀스 입력
- 기대출력:
  - 복구 순서가 반드시 `CB-4 → CB-3 → CB-2 → CB-1`
  - 상위 CB 미복구 상태에서는 하위 CB 완화/해제 동작 보류
- 판정기준(PASS/FAIL):
  - PASS: 상태 전이 로그가 역순 복구 규칙과 정확히 일치
  - FAIL: 역순 위반 또는 하위 CB 조기 복구
